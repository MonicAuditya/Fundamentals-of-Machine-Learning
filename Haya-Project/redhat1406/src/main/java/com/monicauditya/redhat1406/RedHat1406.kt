...
class RedHat1406 {
    companion object {
        init {
            val logTag = RedHat1406::class.java.simpleName
            ...
            if (!isEmulated) {
                if (supportsArm64V8a()) {
                    if (isAtLeastArmV84 && hasSve && hasI8mm && hasFp16 && hasDotProd) {
                        Log.d(logTag, "Loading libredhat1406_v8_4_fp16_dotprod_i8mm_sve.so")
                        System.loadLibrary("redhat1406_v8_4_fp16_dotprod_i8mm_sve")
                    } else if (isAtLeastArmV84 && hasSve && hasFp16 && hasDotProd) {
                        Log.d(logTag, "Loading libredhat1406_v8_4_fp16_dotprod_sve.so")
                        System.loadLibrary("redhat1406_v8_4_fp16_dotprod_sve")
                    } else if (isAtLeastArmV84 && hasI8mm && hasFp16 && hasDotProd) {
                        Log.d(logTag, "Loading libredhat1406_v8_4_fp16_dotprod_i8mm.so")
                        System.loadLibrary("redhat1406_v8_4_fp16_dotprod_i8mm")
                    } else if (isAtLeastArmV84 && hasFp16 && hasDotProd) {
                        Log.d(logTag, "Loading libredhat1406_v8_4_fp16_dotprod.so")
                        System.loadLibrary("redhat1406_v8_4_fp16_dotprod")
                    } else if (isAtLeastArmV82 && hasFp16 && hasDotProd) {
                        Log.d(logTag, "Loading libredhat1406_v8_2_fp16_dotprod.so")
                        System.loadLibrary("redhat1406_v8_2_fp16_dotprod")
                    } else if (isAtLeastArmV82 && hasFp16) {
                        Log.d(logTag, "Loading libredhat1406_v8_2_fp16.so")
                        System.loadLibrary("redhat1406_v8_2_fp16")
                    } else {
                        Log.d(logTag, "Loading libredhat1406_v8.so")
                        System.loadLibrary("redhat1406_v8")
                    }
                } else if (Build.SUPPORTED_32_BIT_ABIS[0]?.equals("armeabi-v7a") == true) {
                    // armv7a (32bit) device
                    Log.d(logTag, "Loading libredhat1406_v7a.so")
                    System.loadLibrary("redhat1406_v7a")
                } else {
                    Log.d(logTag, "Loading default libredhat1406.so")
                    System.loadLibrary("redhat1406")
                }
            } else {
    ...
    object DefaultInferenceParams {
        ...
        val chatTemplate: String =
            "{% for message in messages %}{% if loop.first and messages[0]['role'] != 'system' %}{{ '<|im_start|>system You are a helpful AI assistant named RedHat1406, trained by Haya<|im_end|> ' }}{% endif %}{{'<|im_start|>' + message['role'] + ' ' + message['content'] + '<|im_end|>' + ' '}}{% endfor %}{% if add_generation_prompt %}{{ '<|im_start|>assistant ' }}{% endif %}"
    }
    ...
    /**
     * Unloads the LLM model and releases resources. This method should be called when the RedHat1406
    ...
    private fun verifyHandle() {
        assert(nativePtr != 0L) { "Model is not loaded. Use RedHat1406.create to load the model" }
    }
